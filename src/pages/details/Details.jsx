...
console.log(formData);
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `https://health-track-server-ruddy.vercel.app/doctors/${id}`
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(
        "https://health-track-server-ruddy.vercel.app/service_request",
        data
      );
  ...
  return (
    <section className="min-h-screen ">
      <PageBanner
        title={"Details"}
        pageAddress={`/getAvailableDoctor/${id}`}
        pageDirection={"Details"}
      />
      ...
      <div className="flex flex-col lg:flex-row gap-8 py-10 ">
        ...
        <div className="flex-1 bg-gray-100 p-6 rounded-lg shadow-md">
          ...
          <div className="text-gray-700 mb-4">
            <p>
              <span className="font-semibold">Name:</span> {doctors?.name}
            </p>
            <p>
              <span className="font-semibold">Total Service:</span>{" "}
              {doctors.service_experience}
            </p>
            <p>
              <span className="font-semibold">Specialization:</span>{" "}
              {doctors?.service_category}
            </p>
            <p>
              <span className="font-semibold">Email :</span> {doctors?.email}
            </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {doctors?.availability === "busy" ? (
              <button
                type="submit"
                disabled
                className="text-white py-2 px-4 rounded-lg cursor-not-allowed bg-slate-500 transition-colors"
              >
                Not Allowed
              </button>
            ) : (
              <button
                ...
                className={`text-white py-2 px-4 rounded-lg ${
                  role === "doctor"
                    ...
                    : "cursor-auto bg-[#FD9678] hover:bg-[#1ABC9C]"
